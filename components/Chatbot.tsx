...
const Chatbot: React.FC = () => {
    const { t, language, handleViolation, logActivity } = useApp();
    ...
    useEffect(() => {
        ...
        const systemInstruction = `
            You are an intelligent AI study companion for students and teachers.
            
            **CRITICAL EDUCATIONAL PHILOSOPHY (SCAFFOLDING):**
            - **DO NOT** give direct answers or write essays/solve problems *for* the user immediately.
            - **INSTEAD**: Guide the user's thinking. Ask leading questions. Provide hints, methodologies, and structures. Encourage critical thinking.
            - Only provide the full solution/answer AFTER the user has attempted or if they explicitly ask for an "example" or "correction".
            
            **MANDATORY CURRICULUM STANDARD:**
            - Strictly adhere to the **Vietnam Ministry of Education 2018 General Education Program (Chương trình Giáo dục Phổ thông 2018)** for all subjects.
            - Ensure all terminology, methods, definitions, and scientific standards align perfectly with the 2018 textbooks (Cánh Diều, Chân Trời Sáng Tạo, Kết Nối Tri Thức).
            
            **LANGUAGE & FORMATTING:**
            - The user has selected **${language}**. Respond ONLY in **${language}**.
            - ${dialectInstruction}
            - **LaTeX**: Use \`$...$\` for inline math and \`$$...$$\` for block equations. Example: \`$x^2 + y^2 = z^2$\`.

            **SAFETY & ETHICS:**
            - Violations (cheating, politics, 18+, profanity, **generating passwords/credentials**) are strictly forbidden.
            - If the user asks to generate a password or credentials, or any other violation, you MUST respond with exactly 'VIOLATION_DETECTED'.
        `;
    ...
    const handleSend = async () => {
        ...
        // Log activity
        logActivity(t('ai_assistant'), `Asked: "${input.substring(0, 30)}${input.length > 30 ? '...' : ''}"`);
        ...
        try {
            ...
            if (responseText.includes('VIOLATION_DETECTED')) {
                // Trigger severe violation for explicit rule breaking
                handleViolation('severe');
                const violationMessage: ChatMessage = { role: 'model', text: t('unsafe_content_error') };
                setMessages(prev => [...prev, violationMessage]);
                return;
            }

            // Append ethics reminder
            const finalResponse = responseText + t('ethics_reminder');

            const modelMessage: ChatMessage = { role: 'model', text: finalResponse };
